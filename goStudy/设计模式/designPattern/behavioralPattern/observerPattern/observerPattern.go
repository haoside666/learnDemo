...
type Notifierr interface {
	AddListener(listener Listenerr)
	RemoveListener(listener Listenerr)
	Notify()
}
...
type ClassMonitor struct {
	listenerList []Listenerr //需要通知的全部观察者集合
}

func (m *ClassMonitor) AddListener(listener Listenerr) {
	m.listenerList = append(m.listenerList, listener)
}

func (m *ClassMonitor) RemoveListener(listener Listenerr) {
