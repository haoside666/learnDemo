...
int FilePathParse(const char *sFilePath, char *sDir, char *sFileName)
{
    int i = 0;
...
void encryption(char *p_buf,char *c_buf)
{
    aesEncrypt(key, KEY_LEN, (const uint8_t *)p_buf, (uint8_t *)c_buf, _SIZE_); // 加密
}

void decode(char *p_buf,char *c_buf)
{
    aesDecrypt(key, KEY_LEN, (const uint8_t *)c_buf, (uint8_t *)p_buf, _SIZE_);
}
