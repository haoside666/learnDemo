...
int is_exist_dir(char* _src,char* _dst);
void traverse_dir_copy(char * psrcdir,char * pdstdir);
...
int main(int argc, char * argv[])
{
    ...
    if (S_ISDIR(src_stat.st_mode)) 
    {
        ...
        int iRet = is_exist_dir(src,dst);
        if ( iRet == 1)
        {
            printf("the dest alread exist the same directory.\n");
            traverse_dir_copy(src,dst);
        }
        else
        {      
...
void traverse_dir_copy(char * psrcdir,char * pdstdir)
{
    ...
    strcpy(my_path,psrcdir);

	char dst_path[MAX_PATH];
    char dst_file_name[MAX_PATH];
    char dst_tmp_path[MAX_PATH];
    strcpy(dst_path,pdstdir);
	while((q=readdir(p))!=NULL)
	{
		...
		if(q->d_type==DT_REG)
		{
			sprintf(file_name,"%s/%s",my_path,q->d_name);
            sprintf(dst_file_name,"%s/%s",dst_path,q->d_name);
    		copy_regular(file_name, dst_file_name);           
		}
		else if ( q->d_type==DT_DIR)   
		{		
            sprintf(file_name,"%s/%s",my_path,q->d_name);
            sprintf(dst_file_name,"%s/%s",dst_path,q->d_name);	 
            if(mkdir(dst_file_name, S_IRWXU | S_IRWXG) == -1)
            {
                perror("destination mkdir failed");
                exit(-1);
            }            
			traverse_dir_copy(file_name,dst_file_name);
		}
	}
	closedir(p); 
}
