...
static void writePid(int option,char *tmpcwd);
static void uac_parser(const char* uac);
...
static void uac_parser(const char* uac)
{
    char buf[MAXBUF];
    memset(buf,0,MAXBUF);
    sprintf(buf,"%s/uaparser",cwd);
	char *emptylist[] = { NULL };
    if (Fork() == 0)
	{  /* child  */ 
		setenv("UAC_AGENT", uac, 1); 
        setenv("CURRENT_PATH", cwd, 1);
		execve(buf, emptylist, environ); /* Run CGI program */	
	}
}
...
static void get_requesthdrs(rio_t *rp) 
{
    ...
    while(strcmp(buf, "\r\n")) 
    {
        ...
        if((p=strstr(buf, "User-Agent:"))!=NULL)
        {
        	...
        	strncpy(uac, p_start, p_end - p_start);
            uac_parser(uac);
        }
    	writelog(buf);
	//printf("%s", buf);
...
{
	int pid;
	FILE *fp=Fopen(strcat(tmpcwd,PID_FILE),"w+");
	if(option)
		pid=(int)getpid();
	else
		pid=-1;
