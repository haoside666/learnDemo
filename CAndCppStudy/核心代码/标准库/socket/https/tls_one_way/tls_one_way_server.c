...
void signal_handle(int signal_value)
{
    switch(signal_value)
    {
        case SIGINT:
...
int ssl_set_key_passphrase(char *buf,int size,int rwflag,void *userdata)
{
    return snprintf(buf, size, "%s", server_key_passphrase);
}

int ssl_load_pem(SSL_CTX *ssl_ctx,char *server_passphrase,char *server_key_path,char *server_cert_path,char *server_chain_path)
{
    // 設定伺服器的私鑰的密碼.
...
int ssl_socket_init(struct sockaddr_in *local_addr,socklen_t addr_len,int *sock_fd_buf)
{
    int sock_fd;
    // 建立 socket.
...
int ssl_show_info(struct sockaddr_in *remote_addr,SSL *ssl_session)
{
    X509 *cert_data;
    char data_buf[256];
    DMSG("");
...
int ssl_socket_send(SSL *ssl_session,void *data_con,unsigned int data_len)
{
    int slen;
...
int main(int argc,char **argv)
{
    ...
    while(shutdown_process == 0)
    {
        ...
        // 傳送資料.
        snprintf(data_buf, sizeof(data_buf), "hello, i am server");
