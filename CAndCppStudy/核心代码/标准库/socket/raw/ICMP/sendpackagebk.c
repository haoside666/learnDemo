...
{       
    ...
    ip->ip_hl = sizeof(struct ip)>>2;//首部长度:首部占 32 bit字的数目
    ip->ip_tos = 0;//服务类型
    ip->ip_len = htons(ICMP_PACKET_SIZE);//IP数据报的长度
    ip->ip_id=0;//16位标识
    ip->ip_off=0;//13位片偏移
    ip->ip_ttl=MAXTTL;//8位TTL
    ip->ip_p=IPPROTO_TCP;//8位协议
    ip->ip_sum=0;//16位首部检验和
	inet_pton(AF_INET,localaddr,&ip->ip_src.s_addr);
	inet_pton(AF_INET,destaddr,&ip->ip_dst.s_addr);
	ip->ip_sum=cal_chksum((unsigned short*)ip,sizeof(struct ip));
	...
	int size=sizeof(struct ip)+sizeof(icmp)+strlen(icmp->icmp_data);
	while(1)
	{
...
{       
	...
	while(1)        
	{                      
		if( recvfrom(sockfd,buf,ICMP_PACKET_SIZE,0,(struct sockaddr *)&from,&fromlen) <0)                
		{                        
