...
unsigned char send_CFG_REQ (struct Connection_info *my_connection) 
{
	...
	m_num.Type=0x05;
	m_num.Length=0x06;
	m_num.Data=htons((int)(rand()%2147483647));
	...
	memcpy(lcp_packet.Data,&mru,sizeof(mru));  //MRU
	memcpy(lcp_packet.Data+sizeof(mru),&m_num,sizeof(m_num)); //MAGIC
	// lcp_packet's ready!
	...
	if (send(session_socket,&buff,ETH_HEARER_LEN_WITHOUT_CRC+PPPOE_HEADER_LEN+ntohs(buff.pppoe_length),0)==-1) {
...
int LCP_handle (struct Connection_info *my_connection,struct pppoe_packet *buff,unsigned char LCP_ID) 
{
	...
	if(lcp_packet->Code==0x02 && lcp_packet->Identifier==LCP_ID) {
       ...
       //配置参数类型不支持 rej
	if(lcp_packet->Code==0x04) {
		printf("recv a package of reject\n\n");
        return 0;
	}	
    //参数支持但值不支持 nak
    if(lcp_packet->Code==0x03) {
		printf("recv a package of nak\n\n");
        return 0;
	}	
	//reply configuration request package
...
void send_IPCP_REQ (struct Connection_info *my_connection) {
	...
	ip_address->type=3;
	ip_address->length=6;
	ip_address->data[0]=192;
    ip_address->data[1]=168;
    ip_address->data[2]=1;
    ip_address->data[3]=9;
	//buff's ready!
	int send_count=send(session_socket,&buff,ETH_HEARER_LEN_WITHOUT_CRC+PPPOE_HEADER_LEN+ntohs(buff.pppoe_length),0);
