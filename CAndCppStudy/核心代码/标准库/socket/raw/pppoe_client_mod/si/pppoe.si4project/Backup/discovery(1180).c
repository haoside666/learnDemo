...
static int sendPADI (struct Connection_info *my_connection) 
{
	char dst_mac[MAC_LEN];//mac地址六个字节
	
	struct PPPOE_TAG tag1;
	memset(&tag1,0,sizeof(tag1));
	tag1.TAG_TYPE=htons(Service_Name);
	tag1.TAG_LENGTH=htons(0x0000);
    ...
    //用来标识自己的ID
	my_connection->host_uniq=(unsigned short int)(rand()%65535);
	...
	memset(buff.eth_dst_mac,0xFF,MAC_LEN); //PADI is a broadcast 0xff ff ff ff
...
static int parse_PADO (struct pppoe_packet *PADO,struct Connection_info *my_connection)
{
...
static int recv_PADO (struct Connection_info *my_connection) {
	fd_set readable;
	struct timeval tv;
	...
	while(1) 
    {
        ...
        select_count=select(discovery_socket+1,&readable,NULL,NULL,&tv);
		if (select_count==0) 
        {
		...
		if (select_count==1)
        {
			...
			memset(&buff,0,sizeof(buff));
			ssize_t count;
			count=recv(discovery_socket,&buff,sizeof(buff),0);
			if (count <=0) {
				printf("error in receiving the PADO of client %i\n",my_connection->index);
				return 1;
			}
...
static int sendPADR (struct Connection_info *my_connection) 
{
    struct PPPOE_TAG tag1;
	memset(&tag1,0,sizeof(tag1));
	tag1.TAG_TYPE=htons(Service_Name);
...
static int parse_PADS (struct pppoe_packet *PADS,struct Connection_info *my_connection) {
	if(memcmp(PADS->eth_dst_mac,my_connection->my_mac,MAC_LEN) ==0 && PADS->pppoe_code==CODE_OF_PADS) 
    {
		int i;
...
static int recv_PADS (struct Connection_info *my_connection) {
	fd_set readable;
	struct timeval tv;
	memset(&tv,0,sizeof(tv));
	tv.tv_sec=TIME_OUT_DISCOVERY;
...
void discovery (struct Connection_info *my_connection) {
	...
	for(loop_count=3;loop_count<6;++loop_count) {
		my_connection->my_mac[loop_count]=(int)(rand()%256);
	}
