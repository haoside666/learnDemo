...
static event_handler_t *find_eh(reactor_core_t *core,int fd,int *index)
{
	size_t i = 0;
	event_handler_t *tmp;
...
static int add_eh(reactor_t *self,event_handler_t *eh)
{
	int result = 0;
	...
	if (self->core->current_idx < MAX_USERS)
	{
		...
		printf("current_idx is %ld fd is %d\n",self->core->current_idx,eh->fd);
	}
	return result;
}

static int rm_eh(reactor_t *self,int fd)
{
	int where;
...
static int event_loop(reactor_t *self)
{
	printf("event_loop\n");
	int i,num;
