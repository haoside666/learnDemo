...
def dockerhubchange_sql_result_handler(filename):
    with open(filename, 'r') as file:
        ...
        for line in lines:
            ...
            project_name=l[0].strip()+"/"+l[1].strip()
            num=l[2].strip()
            new_lines.append(f'{project_name} {num}')

    with open("./data/gdChange/cmd_info.txt", 'w') as file:
            file.write('\n'.join(new_lines))

def dockerhubchange_sql_result_handler2(filename):
    with open(filename, 'r') as file:
        lines = file.readlines()
    ...
    with open("./data/gdChange/mType_info.txt", 'w') as file:
            file.write('\n'.join(new_lines))

if __name__=="__main__":
    # gitchange_sql_result_handler("./temp/sql_mType_info.txt")
    # dockerhubchange_sql_result_handler("./temp/sql_record_result.txt")
    dockerhubchange_sql_result_handler2("./temp/sql_mType_info.txt")